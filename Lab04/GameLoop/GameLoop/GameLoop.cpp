...
#include <random>           // gerador de números pseudosaleatórios
...
int pixelX, pixelY;         // coordenadas do pixel
int red, green, blue;       // cores rgb

random_device random_d;
mt19937 mt(random_d());

uniform_int_distribution<int> randWidth(0, windowWidth);
uniform_int_distribution<int> randHeith(0, windowHeight);

uniform_int_distribution<int> randRGB(0, 255);
...
void GameInit()
{
    ...
    // coordenadas aleatórias iniciais
    pixelX = randWidth(mt);
    pixelY = randHeith(mt);

    // cores aleatórias iniciais
    red = randRGB(mt);
    green = randRGB(mt);
    blue = randRGB(mt);
}
...
void GameDraw()
{
    SetPixel(hdc, pixelX, pixelY, RGB(red, green, blue));
}
